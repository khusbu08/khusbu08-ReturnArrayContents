...
public class Lab {
    ...
    public String arrayToString(int[] nums){
        String result = "";
    for (int i = 0; i < nums.length; i++) {
        result += nums[i] + " "; //+= operator to append each integer to the result string, along with a space character
    }
    return result.trim(); //called on the resulting string to remove any trailing whitespace
    }
}
